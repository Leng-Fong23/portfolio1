...
const Home: React.FC = () => {
  ...
  <ul className="flex list-none p-0">
    {navItems.map((item, index) => (
      <li key={index} className="nav-item mr-4 sm:mr-8">
        <a
          onClick={() => handleNavClick(item.link || '')}
          className={`${
            item.title === 'Home'
              ? 'text-blue-500' // Replace with your responsive style for Home
              : item.title === 'Achivement'
              ? 'text-green-500' // Replace with your responsive style for Achivement
              : item.title === 'Hobby'
              ? 'text-red-500' // Replace with your responsive style for Hobby
              : item.title === 'Know More'
              ? 'text-purple-500' // Replace with your responsive style for Know More
              : ''
          } hover:underline`}
        >
          {item.title.replace("'", "&apos;")}
          ...
          <div className="h-screen min-h-screen flex justify-center items-center bg-cover bg-no-repeat bg-center" style={{ backgroundImage: 'url("/download (2).jpeg")' }}>
