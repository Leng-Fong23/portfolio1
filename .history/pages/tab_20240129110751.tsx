...
const SingleTabComponent: React.FC = () => {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
    ...
    <div className="rounded-lg w-96 sm:w-48 md:w-56 lg:w-64 xl:w-64 h-96 sm:h-48 md:h-80 lg:h-64 xl:h-64 bg-[#3A6AF2] flex flex-col items-center justify-center text-white p-2 transition-transform transform-gpu hover:scale-105 shadow-lg">
      ...
      <div
        className={`absolute top-0 left-0 w-full h-full border-2 border-gray-300 rounded-full`}
      ></div>
      <div
        className={`absolute top-0 left-0 w-full h-full border-2 border-blue-500 rounded-full transform rotate-90`}
        style={{
          clip: `rect(0, 1em, 1em, ${30 < 50 ? '0.5em' : 'auto'})`,
        }}
      ></div>
      <div className="absolute top-0 left-0 w-full h-full flex items-center justify-center text-blue-500 font-semibold text-lg">
        {50}%
      </div>
    </div>
    </div>
